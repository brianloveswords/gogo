...
Base.prototype = {
  ...
  validate: function (attr) {
    ...
    function arrayify (o) {
      if (_.isArray(o)) return o; else return [o];
    }
  ...
  get: function (k) {
    ...
    var g, fn = (g = getters[k]) ? g.bind(this) : _.identity;
...
_.extend(Base, {
  ...
  getSchema: function () {
    ...
    _.each(schema, function (value, key) {
      ...
      if (_.isString(value))
        newSchema[key] = _.extend({}, {sql: value});

      // Pass objects straight through
      if (_.isObject(value))
        newSchema[key] = _.extend({}, value);
      ...
      if (_.isFunction(value)) {
        ...
        newSchema[key] = _.extend({}, newSchema[key], spec);
      }
      _.defaults(newSchema[key], {mutators: {}, validators: []});
    });
  ...
  makeTable: function (callback) {
    ...
    this.client.query(this.createTableSql(schema), callback);
  },

  updateSchemaVersionTable: function (callback) {
    ...
    var modelTable = this.table();
    var version = this.version() || '0000';
    ...
    client.query(sql, function (err, result) {
      ...
      // switch to using an update. if there's any other error, use
      // `handleError` to either send it to the callback or throw it.
      var data = { table: modelTable, version: version };
      return client.insert(schemaTable, data, function (err, result) {
        if (!err) return callback(null, result);
        if (!err.message.match(/duplicate/i)) return handleError(err);
