...
Base.prototype = {
  ...
  save: function save(callback) {
    ...
    this.client.upsert(this.table, data, function (err, result) {
      ...
      if (!self.get('id')) {
        self.constructor.findById(result.insertId, function (err, saved) {
          if (err) return callback(err);
...
_.extend(Base, {
  ...
  createTableSql: function (schema) {
    ...
    if (!table) {
      var msg = 'Table must be specified before trying to generate the `create table` statement';
      throw new Error(msg);
    }
    ...
    if (this.prototype.driver === 'mysql') {
      statements.push(_.upcase('engine ='), engine);
    }
  ...
  makeTable: function (callback) {
    callback = (callback || errorThrower).bind(this);
  ...
  truncateTable: function (callback) {
    // #TODO: use truncate if there is no foreign key constraint
    callback = (callback || errorThrower).bind(this);
    var sql = _.strjoin('delete from',  _.backtick(this.prototype.table));
    this.client.query(sql, function (err, res) {
      if (err) return callback(err);
      callback(null, this);
    }.bind(this));
