...
module.exports = function (client) {
  ...
  return function MigrationRunner (migrations) {
    ...
    QueryRunner.stack = [];
    QueryRunner.queue = function queue (args) {
      this.stack.push([].slice.call(args));
    };
    // either emit a 'done' event if the stack is empty or continue flushing.
    QueryRunner.checkDone = function checkDone () {
      if (!this.stack.length) return QueryRunner.emit('done')
    ...
    helpers.client.query = function query () {
      ...
      return QueryRunner.queue(arguments);
    };
    ...
    function runMigration (direction) {
      return function (version, callback) {
        ...
        QueryRunner.once('done', function (err) {
          if (err) return callback(err);
          this.updateSchemaVersion(version, callback);
        }.bind(this));
